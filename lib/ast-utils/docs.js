...
function copyProperties(from, to, behaviorsByName) {
  Object.keys(from.properties).forEach(function(key){
    var newProps;
    if (key in to.properties && !to.properties[key].__fromBehavior) {
      return;
    }
    var newProp = {__fromBehavior: true};
    var fromProp = from.properties[key];
    ...
    Object.keys(fromProp).forEach(function(propertyField){
      ...
      console.log(propertyField, newProp[propertyField]);
    });
  });
  if (!from.behaviors) {
    return;
  ...
  from.behaviors.forEach(function(behavior){
    ...
    console.log(behavior, definedBehavior, Object.keys(behaviorsByName));
...
function annotateElement(descriptor, behaviorsByName) {
  ...
  if (descriptor.behaviors) {
    console.log(descriptor.is, descriptor.behaviors);
    descriptor.behaviors.forEach(function(behavior){
      if (!behaviorsByName[behavior]) {
        console.log(Object.keys(behaviorsByName));
